...
public class Usuario_DAO implements Validar{
    static ArrayList<Observation> cachedObservations = null;
    Connection con;
    Conexion_DB cn = new Conexion_DB();
    PreparedStatement ps;
    ResultSet rs;
    int r,ID;
    static String url = "http://192.168.1.142:8080/fhir";
...
public static ArrayList<Patient> getPatientsData() throws JsonMappingException, JsonProcessingException{
        javax.ws.rs.client.Client client = ClientBuilder.newClient();
        Response response = client.target(url)
        ...
        for (int i = 0; ; ++i) {
            ...
            try {
                id = node.get("entry").get(i).get("resource").get("id").asText();
            } catch (Exception e) {             
                break;
            }
...
public static ArrayList<Observation> getObservationsData() throws JsonMappingException, JsonProcessingException{
    ...
    public static int getStatistics(List<Condition> conditions, String requestor) throws JsonProcessingException{
        ArrayList<Patient> patients = getPatientsData();
